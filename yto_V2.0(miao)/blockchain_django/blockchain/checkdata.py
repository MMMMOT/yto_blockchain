...
def check_new_data():
    ...
    #time = timezone.localtime(timezone.now()).strftime("%Y-%m-%d %H:%M:%S")#当前时间为标准
    last_time_str = datetime_to_iso8601(str(last_time))#最新时间的正常格式，字符串
    get_url = "http://112.65.156.58:9000/v2/odata/datapoints?$filter=DeviceNumber eq '000000004' "+"and dpCreationTime gt "+last_time_str+"&$orderby=dpCreationTime asc"

    r = requests.get(get_url)
    rjson = r.json()
    rlist = rjson['value']#包含着所有过包信息的列表，列表元素为字典，每个字典代表一个过包记录？
   # rlist.sort(key = lambda x:x["DpCreationTime"])#对列表内的单号（字典）信息按"DpCreationTime"（创建时间）进行排序
    #遍历整个列表，生成区块
